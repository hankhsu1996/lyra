...
- DPI import/export (partial: `import "DPI-C" pure function` with 2-state scalar/real/shortreal/string, JIT only; no 4-state, export, context, tasks, AOT)
...
- **Edge-trigger granularity**: `@(posedge ...)` / `@(negedge ...)` support plain signal variables, constant bit/element-selects on packed types (e.g., `@(posedge bus[3])`, `@(posedge bus[1])` on multi-dimensional packed arrays), dynamic bit-selects on packed types with variable indices including compound expressions (e.g., `@(posedge bus[i])`, `@(posedge bus[i+j])`, `@(posedge bus[i*2+1])`), dynamic indexed part-selects on packed types (e.g., `@(posedge bus[i +: 4])`, `@(posedge bus[i -: 4])`), packed struct fields of any width (e.g., `@(posedge s.b)` where `b` is multi-bit), unpacked struct fields of any width (e.g., `@(posedge s.data)` where `data` is `logic [7:0]`), unpacked array elements with constant or dynamic index of any width (e.g., `@(posedge arr[0])`, `@(posedge arr[i])`, `@(posedge arr[i+j])` where `arr` is a fixed-size unpacked array), and constant range/part-selects on packed types (e.g., `@(posedge bus[7:4])`, `@(posedge bus[3 +: 4])`). For multi-bit sub-expressions (packed or unpacked), edge detection samples the expression LSB (`expr[0]`). Level-sensitive waits (`@(expr)` with no edge keyword) observe the full expression. Edge triggers on dynamic array elements and queue elements with constant or dynamic index (e.g., `@(posedge d[i])`, `@(posedge q[i+j])`) are supported; the subscription chases the container handle to read element data from the heap. Division (`/`) and modulo (`%`) are supported in index expressions; division by zero deactivates the subscription (no crash, no spurious trigger). Edge triggers on index variables wider than 64 bits are rejected at compile time. 4-state index variables with X/Z bits deactivate the subscription until the unknown bits clear.
