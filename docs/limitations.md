...
- **Edge-trigger granularity**: `@(posedge ...)` / `@(negedge ...)` support plain signal variables, constant bit/element-selects on packed types (e.g., `@(posedge bus[3])`, `@(posedge bus[1])` on multi-dimensional packed arrays), dynamic bit-selects on packed types with a single variable index (e.g., `@(posedge bus[i])`), packed struct fields of any width (e.g., `@(posedge s.b)` where `b` is multi-bit), unpacked struct fields of any width (e.g., `@(posedge s.data)` where `data` is `logic [7:0]`), constant-index unpacked array elements of any width (e.g., `@(posedge arr[0])` where `arr` is `logic [7:0] arr[0:3]`), and constant range/part-selects on packed types (e.g., `@(posedge bus[7:4])`, `@(posedge bus[3 +: 4])`). For multi-bit sub-expressions (packed or unpacked), edge detection samples the expression LSB (`expr[0]`). Level-sensitive waits (`@(expr)` with no edge keyword) observe the full expression. Edge triggers on compound index expressions (`bus[i+j]`), dynamic array/queue elements, dynamic unpacked array indices, dynamic indexed part-selects, and index variables wider than 64 bits are rejected at compile time. 4-state index variables with X/Z bits deactivate the subscription until the unknown bits clear.
