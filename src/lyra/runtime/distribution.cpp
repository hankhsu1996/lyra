...
auto Uniform(DistInt& seed, DistInt start, DistInt end) -> double {
...
auto Normal(DistInt& seed, DistInt mean, DistInt deviation) -> double {
  double first = 0.0;
  double second = 0.0;
  double radius = 1.0;
...
auto Exponential(DistInt& seed, DistInt mean) -> double {
...
auto Poisson(DistInt& seed, DistInt mean) -> DistInt {
  DistInt count = 0;
...
auto ChiSquare(DistInt& seed, DistInt degrees_of_freedom) -> double {
  double total = 0.0;
...
auto StudentT(DistInt& seed, DistInt degrees_of_freedom) -> double {
...
auto Erlangian(DistInt& seed, DistInt stages, DistInt mean) -> double {
  double product = 1.0;
...
// at a half.
auto RoundDraw(double draw) -> DistInt {
...
auto TruncateDraw(double draw) -> DistInt {
...
auto UniformIn(DistInt& seed, DistInt start, DistInt end) -> DistInt {
  if (start >= end) {
    return start;
  }
...
auto DistExponential(
  ...
  DistInt state = IntegerArgument(seed);
  const double drawn = Exponential(state, shape);
  return DrawnWith(RoundDraw(drawn), state);
}
...
auto DistPoisson(const value::PackedArray& seed, const value::PackedArray& mean)
  ...
  DistInt state = IntegerArgument(seed);
  const DistInt drawn = Poisson(state, shape);
  return DrawnWith(drawn, state);
}
...
auto DistChiSquare(
  ...
  DistInt state = IntegerArgument(seed);
  const double drawn = ChiSquare(state, shape);
  return DrawnWith(RoundDraw(drawn), state);
}
...
auto DistT(
  ...
  DistInt state = IntegerArgument(seed);
  const double drawn = StudentT(state, shape);
  return DrawnWith(RoundDraw(drawn), state);
}
...
auto DistErlang(
  ...
  RequirePositive(average, "$dist_erlang", "mean");
  DistInt state = IntegerArgument(seed);
  const double drawn = Erlangian(state, stage_count, average);
  return DrawnWith(RoundDraw(drawn), state);
}
