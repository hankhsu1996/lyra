...
- [ ] System tasks -- the `$display` / `$write` / `$strobe` / `$monitor` family, file IO,
      `$sformat`, `$time` / `$timeformat`, `$random`, `$readmem*` / `$writemem*`. Formatting is
      tracked in `display.md`, memory file load / dump in `mem_file.md`, and RNG in
      `simulation-rng.md`; most land in the runtime call registry. `$readmem{h,b}` and
      `$writemem{h,b}` handle a one-dimensional memory of packed elements today; multidimensional
      and associative / dynamic memories are the remaining container-shape gap (`mem_file.md`).
      Other known gaps: `$monitor`, `$random`. (The archive effect / pure / state classification is
      retired -- see below.)
