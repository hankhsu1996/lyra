...
// to the last. N is an int unsigned, so it may be larger than any int, and it
// still counts back that many names. A value that is not one of the names has no place in that
...
module Top;
  ...
  initial begin
    ...
    // 2**32 - 1 names back is a whole number of rounds of five.
    v = C;
    step = 32'hFFFF_FFFF;
    before_largest_step = v.prev(step);
  ...
  final begin
    ...
    if (before_largest_step !== 9)
      $fatal(1, "before_largest_step was %0d, expected 9", before_largest_step);
