...
- [ ] B -- Recipe model: non-local access, connections, construction (see [compilation-model.md](../compilation-model.md))
  ...
  - [ ] B2 -- Compile-time recipe lowering: hierarchical refs to external ref handles, connections to body-local recipes, parent-child port via CompiledModuleHeader; delete cross_instance_places, ResolveHierarchicalRef, InstanceSlotResolver, ResolvedBindingPlan, backend clone/remap
