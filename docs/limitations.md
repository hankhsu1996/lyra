...
- **Edge-trigger granularity**: `@(posedge ...)` / `@(negedge ...)` support plain signal variables, constant bit-selects on packed types (e.g., `@(posedge bus[3])`), single-bit struct fields on packed or unpacked structs (e.g., `@(posedge s.flag)`), constant-index unpacked array elements with single-bit type (e.g., `@(posedge arr[0])`), and single-bit constant range/part-selects (e.g., `@(posedge bus[3:3])`, `@(posedge bus[3 +: 1])`). Edge triggers on multi-bit sub-expressions, dynamic selects, dynamic array/queue elements, and dynamic unpacked array indices are rejected at compile time. Use `@(*)` or `@(signal)` for level-sensitive observation of unsupported sub-expressions.
