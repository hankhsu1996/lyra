...
// body reached this way takes the value as its first parameter: nothing on the
...
// order and shared by every value of the class. It crosses as a pointer plus a
...
// no value carries one, because such a class is never constructed.
...
// the position is the value's to answer and is not part of this.
...
// declares is what runs, whatever the value turns out to be (LRM 8.14), so
// nothing about the call is left for the value to answer and the answer is the
// address itself rather than a position to look one up at. A method answering a
// dispatch position is not here: for one of those the value decides, and what
...
// Where one property of a class lives on a value of that class, answered by
// whoever laid the value out. It is the whole of what a target has to supply
// about reaching a property: everything else -- which class declares it, how a
// value of a class extending that one gets to it -- is the same question
...
// The same value seen as the class this one extends (LRM 8.13). Only the side
// that laid the value out can say what that is, which is why it is an entry
...
// How a value of one class answers where the property `declared_by` gave
...
// takes knowing how the value was laid out -- which is the one thing the
// asking side never has.
using PropertyAccessor = void* (*)(GcObject * object,
                                   const ObjectDefinition* declared_by,
                                   std::uint32_t slot);

// Which body a value answers `introduced_by`'s `ordinal`-th behavior with
// (LRM 8.20, 8.22). Nothing about the value beyond its class takes part, every
// value of one class answering alike, but which class it is of is the value's
// own to say.
using BehaviorAccessor = ErasedMethodEntry (*)(
    GcObject* object, const ObjectDefinition* introduced_by,
    std::uint32_t ordinal);

// The two answers a class gives when nothing has laid its values out flat:
// each walks what the class extends, taking the value's own class as the
...
// entries by which a value of it is reached, and whatever a realization that
// lays its values out flat adds. A class joins no lifecycle and holds no place
// in the object tree, and which body initializes a value is settled where the
// value is asked for rather than by the class it is of (LRM 8.7), so what a
// definition carries is what every value of the class shares and nothing about
// any one of them.
//
...
// values of a class either way, and the questions a definition answers -- where
// does this property live, which body fills this position -- are the same
// questions of both.
//
// Two things a definition never states, and both are why it answers through
// entries. Where a property sits in a value is the answer of whoever laid that
// value out, which for one target is another language's compiler. And what a
// class's lineage adds up to is a fact no unit can see whole, since a base may
// be declared past the boundary -- so a class states its own contribution and
// the answers walk.
//
...
struct ObjectDefinition {
  ...
  // What a realization that lays every value of the class out flat adds, and
  ...
  // What a target that lays every value of the class out itself supplies, and
...
// The definition of one class whose values stand in the design hierarchy: what
// every class definition carries, and what the runtime additionally needs to
// drive one -- the program it enters values through, held by value so a class's
// whole generated behavior is one constant, and its structural-construction
// entry.
//
// It is a class definition rather than a record standing beside one because a
// hierarchy instance is a value of a class like any other: one address serves
// both the site that constructs a value of it and the site that dispatches on
// one, and the storage schema is stated once. Construction is
...
struct ScopeDefinition : ObjectDefinition {
...
// The body a value of `cls` answers `introduced_by`'s `ordinal`-th behavior
// with (LRM 8.20), read off the table its lineage left flat. Which class a
// value is, is the value's own to answer; this is that class answering, so what
// kind of value carries it makes no difference and every kind reaches the one
// statement of it here.
[[nodiscard]] auto BodyOf(
...
// The body `cls` answers `name` with, for a call that leaves the value nothing
...
[[nodiscard]] auto PropertyAt(GcObject* object, const PropertyCoordinate* at)
    -> void*;
[[nodiscard]] auto BehaviorAt(GcObject* object, const BehaviorCoordinate* at)
    -> ErasedMethodEntry;

[[nodiscard]] inline auto PropertyAt(
    const value::ManagedRef& handle, const PropertyCoordinate* at) -> void* {
  return PropertyAt(static_cast<GcObject*>(handle.Share().get()), at);
}

[[nodiscard]] inline auto BehaviorAt(
    const value::ManagedRef& handle, const BehaviorCoordinate* at)
    -> ErasedMethodEntry {
  return BehaviorAt(static_cast<GcObject*>(handle.Share().get()), at);
}
