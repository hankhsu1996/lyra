...
void HashOperand(
  ...
  switch (op.kind) {
    ...
    case mir::Operand::Kind::kUse: {
      ...
      HashPlaceId(h, place_id, arena, types);
      break;
    }
...
void HashPlace(
    ...
    const TypeArena& types) {
  // Hash root: kModuleSlot hashes body-local id directly (already 0-based),
  // kDesignGlobal hashes global id directly (stable across instances).
...
void HashFormatOp(
  ...
  if (op.value) {
    h.MixBool(true);
    HashOperand(h, *op.value, arena, types);
  } else {
    h.MixBool(false);
  }
  ...
  std::visit(
      [&h, &arena, &types](const auto& info) {
        ...
        if constexpr (std::is_same_v<T, mir::UnaryRvalueInfo>) {
          ...
          if (info.receiver) {
            HashPlaceId(h, *info.receiver, arena, types);
          }
          ...
          h.Mix8(7);
          HashPlaceId(h, info.place, arena, types);
          HashTypeShape(h, info.result_type, types);
          ...
          h.Mix8(11);
          HashTypedOperand(h, info.query, arena, types);
        } else if constexpr (std::is_same_v<T, mir::FopenRvalueInfo>) {
          h.Mix8(12);
          HashTypedOperand(h, info.filename, arena, types);
          h.MixBool(info.mode.has_value());
          if (info.mode) HashTypedOperand(h, *info.mode, arena, types);
          ...
          if (info.command) {
            HashTypedOperand(h, *info.command, arena, types);
          }
        }
      },
      rv.info);
...
void HashRhs(
    ...
    const TypeArena& types) {
  std::visit(
      [&h, &arena, &types](const auto& val) {
        using T = std::decay_t<decltype(val)>;
        if constexpr (std::is_same_v<T, mir::Operand>) {
          h.Mix8(0);
          HashOperand(h, val, arena, types);
        } else if constexpr (std::is_same_v<T, mir::Rvalue>) {
          h.Mix8(1);
          HashRvalue(h, val, arena, types);
        }
      },
      rhs);
}
      ...
      [&h, &arena, &types](const auto& op) {
        ...
        if constexpr (std::is_same_v<T, mir::DisplayEffect>) {
          ...
          for (const auto& fmt : op.ops) {
            HashFormatOp(h, fmt, arena, types);
          }
          h.MixBool(op.descriptor.has_value());
          if (op.descriptor) {
            HashOperand(h, *op.descriptor, arena, types);
          }
          ...
          h.MixBool(op.is_hex);
          HashPlaceId(h, op.target, arena, types);
          HashTypeShape(h, op.target_type, types);
          HashTypedOperand(h, op.filename, arena, types);
          ...
          h.Mix8(8);
          HashPlaceId(h, op.target, arena, types);
          HashOperand(h, op.fill_value, arena, types);
...
void HashStatement(
  ...
  std::visit(
      [&h, &arena, &types, &module_functions](const auto& data) {
        using T = std::decay_t<decltype(data)>;
        if constexpr (std::is_same_v<T, mir::Assign>) {
          h.Mix8(0);
          HashPlaceId(h, data.dest, arena, types);
          HashRhs(h, data.rhs, arena, types);
        } else if constexpr (std::is_same_v<T, mir::GuardedAssign>) {
          h.Mix8(1);
          HashPlaceId(h, data.dest, arena, types);
          HashRhs(h, data.rhs, arena, types);
          HashOperand(h, data.guard, arena, types);
        } else if constexpr (std::is_same_v<T, mir::Effect>) {
          h.Mix8(2);
          HashEffect(h, data.op, arena, types);
          ...
          if (data.ret) {
            HashPlaceId(h, data.ret->tmp, arena, types);
            h.MixBool(data.ret->dest.has_value());
            if (data.ret->dest) {
              HashPlaceId(h, *data.ret->dest, arena, types);
            }
          ...
          for (const auto& wb : data.writebacks) {
            ...
            if (wb.tmp) {
              HashPlaceId(h, *wb.tmp, arena, types);
            }
            HashPlaceId(h, wb.dest, arena, types);
          ...
          HashPlaceId(h, data.receiver, arena, types);
          ...
          std::visit(
              [&h, &arena, &types](const auto& op) {
                ...
                if constexpr (std::is_same_v<U, mir::AssocGet>) {
                  h.Mix8(0);
                  HashPlaceId(h, op.dest, arena, types);
                  HashOperand(h, op.key, arena, types);
                  ...
                  h.Mix8(6);
                  HashPlaceId(h, op.dest_found, arena, types);
                  HashPlaceId(h, op.out_key, arena, types);
                  ...
                  h.Mix8(8);
                  HashPlaceId(h, op.dest_found, arena, types);
                  HashPlaceId(h, op.key_place, arena, types);
                  ...
                  h.Mix8(10);
                  HashPlaceId(h, op.dest_keys, arena, types);
                }
              },
              data.data);
        }
...
void HashWaitTrigger(
    ...
    const TypeArena& types) {
  // Hash signal ref: scope + id directly.
  // kModuleLocal is already 0-based; kDesignGlobal is stable across instances.
  h.Mix8(static_cast<uint8_t>(trigger.signal.scope));
  h.Mix32(trigger.signal.id);
  h.Mix8(static_cast<uint8_t>(trigger.edge));
  h.MixBool(trigger.observed_place.has_value());
  if (trigger.observed_place) {
    HashPlaceId(h, *trigger.observed_place, arena, types);
  }
  h.MixBool(trigger.late_bound.has_value());
  if (trigger.late_bound) {
    const auto& lb = *trigger.late_bound;
    h.Mix(lb.dep_slots.size());
    for (const auto& slot : lb.dep_slots) {
      h.Mix8(static_cast<uint8_t>(slot.scope));
      h.Mix32(slot.id);
    }
  ...
  std::visit(
      [&h, &arena, &types](const auto& data) {
        ...
        if constexpr (std::is_same_v<T, mir::Jump>) {
          ...
          h.Mix8(1);
          HashOperand(h, data.condition, arena, types);
          ...
          h.Mix8(2);
          HashOperand(h, data.selector, arena, types);
          ...
          for (const auto& trigger : data.triggers) {
            HashWaitTrigger(h, trigger, arena, types);
          }
          h.Mix32(data.resume.value);
          ...
          if (data.value) {
            HashOperand(h, *data.value, arena, types);
          }
          ...
          if (data.message) {
            HashOperand(h, *data.message, arena, types);
          }
...
auto ComputeProcessFingerprint(
  ...
  for (const auto& block : process.blocks) {
    ...
    for (const auto& stmt : block.statements) {
      HashStatement(h, stmt, arena, types, module_functions);
    }

    // Hash terminator
    HashTerminator(h, block.terminator, arena, types);
  }
