...
// in hexadecimal and ten in decimal, and one column more than that where the
// type is signed and the sign needs one of its own. A field width written
// between the % and the radix letter overrides that sizing; it is a
// non-negative decimal constant, so a leading zero belongs to the number and
// asks for nothing of its own, and a width of zero asks for the minimum width
// with no padding at all. A value narrower than its field is padded on the
// left -- with spaces where it is decimal or a string, and with zeros where it
// is written in another radix -- and a value wider than its field expands to
// fit rather than being truncated (LRM 21.2.1.2).
module Top;
  int negative;

  string sized_decimal;
  string sized_signed_decimal;
  string sized_hex;
  string min_decimal;
  string min_hex;
  string min_radices;
  ...
  string wide_string;
  string zero_prefixed_decimal;
  string zero_prefixed_hex;

  initial begin
    negative = -5;

    sized_decimal = $sformatf("%d", 32'd10);
    sized_signed_decimal = $sformatf("%d", negative);
    ...
    zero_prefixed_decimal = $sformatf("%05d", 32'd5);
    zero_prefixed_hex = $sformatf("%08h", 32'h5);
  end
  ...
  final begin
    if (sized_decimal != "        10")
      $fatal(1, "an unsized decimal was '%s', expected 10 right-justified",
             sized_decimal);
    if (sized_signed_decimal != "         -5")
      $fatal(1,
             "an unsized signed decimal was '%s', expected -5 in 11 columns",
             sized_signed_decimal);
    ...
    if (zero_prefixed_decimal != "    5")
      $fatal(1,
             "decimal 5 in a field of 05 was '%s', expected four spaces then 5",
             zero_prefixed_decimal);
    if (zero_prefixed_hex != "00000005")
      $fatal(1, "hex 5 in a field of 08 was '%s', expected 00000005",
             zero_prefixed_hex);
