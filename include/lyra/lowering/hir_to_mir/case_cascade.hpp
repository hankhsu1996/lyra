...
// Appends the SV-case selector snapshot to `frame.current_procedural_scope`:
...
// reached via `frame.current_procedural_scope` and returns the final predicate
...
// scope reached via `label_frame.current_procedural_scope` and returns its
...
auto BuildEqualityChain(
  ...
  auto& enc_scope = *frame.current_procedural_scope;
...
// Assembles the SV-case if/else-if cascade. `wrapper_scope` already holds the
...
// the cascade lives in; it carries the parent procedural scope, depth, and
// closure context so per-level predicate scopes get the right composition when
...
// current_procedural_scope points at the scope into which the predicate is
// lowered (wrapper_scope for the outermost item, a fresh intermediate scope
// for the rest). sel_hops gives the hop count from that scope to the one
...
auto BuildCaseCascade(
    WalkFrame frame, mir::ProceduralScope wrapper_scope,
  ...
  for (std::size_t i = item_count; i-- > 1;) {
    ...
    const mir::ProceduralScopeId body_scope_id =
        level_scope.AddChildScope(std::move(body_scopes[i]));
  ...
  if (item_count > 0) {
    ...
    const mir::ProceduralScopeId body0_id =
        wrapper_scope.AddChildScope(std::move(body_scopes[0]));
    std::optional<mir::ProceduralScopeId> else0_id;
    if (tail.has_value()) {
      else0_id = wrapper_scope.AddChildScope(std::move(*tail));
    }

    wrapper_scope.AppendStmt(
        mir::Stmt{
            .label = std::nullopt,
            .data = mir::IfStmt{
                .condition = *pred0_or,
    ...
    const mir::ProceduralScopeId def_id =
        wrapper_scope.AddChildScope(std::move(*tail));
    wrapper_scope.AppendStmt(
        mir::Stmt{
  ...
  const mir::ProceduralScopeId wrapper_scope_id =
      frame.current_procedural_scope->AddChildScope(std::move(wrapper_scope));
