...
// the elements in the order the dimension declares them, a replication stands
// for an entire dimension, and the default key reaches every element at once.
// Because the array is packed, what results is the vector its elements form,
// the first expression landing in the most significant element. A vector of
// bits is such an array, so a pattern fills it a bit at a time
// (LRM 10.9, 10.9.1, 7.4.1).
module Top;
