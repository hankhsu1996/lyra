...
- **Edge-trigger granularity**: `@(posedge ...)` / `@(negedge ...)` support plain signal variables, constant bit/element-selects on packed types (e.g., `@(posedge bus[3])`, `@(posedge bus[1])` on multi-dimensional packed arrays), packed struct fields of any width (e.g., `@(posedge s.b)` where `b` is multi-bit), unpacked struct fields of any width (e.g., `@(posedge s.data)` where `data` is `logic [7:0]`), constant-index unpacked array elements of any width (e.g., `@(posedge arr[0])` where `arr` is `logic [7:0] arr[0:3]`), and constant range/part-selects on packed types (e.g., `@(posedge bus[7:4])`, `@(posedge bus[3 +: 4])`). For multi-bit sub-expressions (packed or unpacked), edge detection samples the expression LSB (`expr[0]`). Level-sensitive waits (`@(expr)` with no edge keyword) observe the full expression. Edge triggers on dynamic selects, dynamic array/queue elements, and dynamic unpacked array indices are rejected at compile time.
