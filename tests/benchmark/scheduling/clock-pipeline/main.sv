...
// @work: cycle
//
// An eight-stage pipeline, each stage a combinational block feeding a flop, so
// a cycle here pays a clock edge, a nonblocking commit per stage, and a
// propagation pass down the chain. It is the closest of these cases to what an
// ordinary synchronous design costs.
...
module Top;
  ...
  always @(posedge clk) begin
    if (cycle_count == 32'(num_cycles)) begin
      $display("clock-pipeline done: %0d cycles", cycle_count);
      $finish;
    end
  end
endmodule
