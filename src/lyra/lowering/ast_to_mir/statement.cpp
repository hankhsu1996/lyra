...
auto LowerReadWriteMemStatement(
    const slang::ast::CallExpression& call_expression)
    -> std::unique_ptr<mir::Statement> {
  auto name = call_expression.getSubroutineName();
  bool is_read = name == "$readmemh" || name == "$readmemb";
  bool is_write = name == "$writememh" || name == "$writememb";
  if (!is_read && !is_write) {
    return nullptr;
  }

  auto format = (name == "$readmemb" || name == "$writememb")
                    ? mir::MemFileFormat::kBin
                    : mir::MemFileFormat::kHex;
  ...
  auto filename = LowerExpression(*args[0]);
  ...
  std::unique_ptr<mir::Expression> start_expr;
  std::unique_ptr<mir::Expression> end_expr;
  if (args.size() >= 3) {
    start_expr = LowerExpression(*args[2]);
    if (start_expr->type.kind != common::Type::Kind::kIntegral) {
      throw DiagnosticException(
          Diagnostic::Error(
              args[2]->sourceRange,
  ...
  if (args.size() == 4) {
    end_expr = LowerExpression(*args[3]);
    if (end_expr->type.kind != common::Type::Kind::kIntegral) {
      throw DiagnosticException(
          Diagnostic::Error(
              args[3]->sourceRange,
  ...
  if (is_read) {
    return std::make_unique<mir::ReadMemStatement>(
        format, &named_value.symbol, target_type, std::move(filename),
        std::move(start_expr), std::move(end_expr));
  }
  return std::make_unique<mir::WriteMemStatement>(
      format, &named_value.symbol, target_type, std::move(filename),
      std::move(start_expr), std::move(end_expr));
}
...
auto LowerExpressionStatement(const slang::ast::Expression& expr)
  ...
  if (expr.kind == slang::ast::ExpressionKind::Call) {
    const auto& call_expression = expr.as<slang::ast::CallExpression>();
    if (call_expression.isSystemCall()) {
      if (auto stmt = LowerReadWriteMemStatement(call_expression)) {
        return stmt;
      }
    }
  }
