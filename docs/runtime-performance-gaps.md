...
1. **Signal flush** -- 10.8% self. Subscription traversal + snapshot refresh. Now the single largest self-cost function. G5b landed dense typed subscriber storage (EdgeSub/ChangeSub/RebindWatcherSub/ContainerSub with separated cold pools). G5c landed compile-time trigger descriptors: `WaitTriggerRecord` now carries explicit `kind` (edge/change/container), `flags` (initially-active), and `container_elem_stride`. Runtime installs subscriptions directly from these descriptors via canonical `Engine::InstallTriggers()` -- no more scanning late-bound headers to classify trigger shape, no `byte_size == UINT32_MAX` OOB sentinel, no post-install flag patching. Both `HandleSuspendRecord` and `InstallWaitSite` delegate to the same install path. Remaining: snapshot refresh optimization, per-kind flush tuning.
