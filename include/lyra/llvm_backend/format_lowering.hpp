...
//
// Usage:
//   return WithStringHandle(ctx, facts, op, type,
...
auto WithStringHandle(
    Context& context, const CuFacts& facts, SlotAccessResolver& resolver,
    const mir::Operand& operand, TypeId type_id, F&& body) -> Result<void>;
...
auto LowerFormatOpToBuffer(
    Context& context, const CuFacts& facts, llvm::Value* buf,
...
auto WithStringHandle(
    Context& context, const CuFacts& facts, const mir::Operand& operand,
    TypeId type_id, F&& body) -> Result<void> {
  auto handle_result = LowerArgAsStringHandle(context, facts, operand, type_id);
...
auto WithStringHandle(
    ...
    const mir::Operand& operand, TypeId type_id, F&& body) -> Result<void> {
  auto handle_result =
      LowerArgAsStringHandle(context, facts, resolver, operand, type_id);
  if (!handle_result) return std::unexpected(handle_result.error());
  auto [handle, needs_release] = *handle_result;
