...
// The constant's numeric value in one machine integer, sign-extended from its
// declared width. Only reached where the width fits the carrier and no bit is
// X or Z, so nothing is lost.
auto CarrierValue(const mir::IntegralConstant& c) -> std::int64_t {
...
auto BuildWordPlane(
  ...
  return block.exprs.Add(
      mir::Expr{
          ...
          .type =
              unit.types.MachineArrayOf(unit.builtins.machine_word, count)});
}
...
auto BuildIntegralLiteral(
  ...
  if (value.width <= 64U && !has_unknown) {
    const mir::ExprId carrier = block.exprs.Add(
        mir::Expr{
            .data = mir::MachineIntLiteral{.value = CarrierValue(value)},
  ...
  const bool is_four_state =
      value.state_kind == mir::IntegralStateKind::kFourState;
  if (has_unknown && !is_four_state) {
    throw InternalError(
        "BuildIntegralLiteral: a 2-state constant cannot carry an X or Z bit");
  }
  ...
  const std::size_t words = (value.width + 63U) / 64U;
