...
// carries, and has the class's construction hand each object that record's
// address.
//
...
//
// So the record is what the class says about its own objects, in three parts
// that side alone can answer: where each property it declares sits on one, how
// one is seen as the class it extends, and which body answers each behavior.
// Everything else -- which class declares what, how a class extending it gets
// to that class -- is the same question whatever it is spelled in, and is
// answered by walking rather than by anything stated here.
