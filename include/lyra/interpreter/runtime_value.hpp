...
struct RuntimeValue {
 public:
  ...
  static auto Pointer(common::Type pointer_type, Address addr) -> RuntimeValue {
    return RuntimeValue{
        .type = std::move(pointer_type), .value = std::move(addr)};
  }
  ...
  [[nodiscard]] auto AsAddress() const -> const Address& {
    ...
    auto copied_value = std::visit(
        [](const auto& v) -> ValueVariant {
          ...
          if constexpr (std::is_same_v<T, UninitValue>) {
            ...
            // Address values are copyable - just copy
            return v;
          }
        },
        value);
