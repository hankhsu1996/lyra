...
// references a block-local sink in that body. Lvalue semantics
// (observable members, partial selectors, etc.) belong to the
...
struct IntegralScanSlot {
  ExprId temp{};  // LocalRef to a PackedArray temp
...
struct StringScanSlot {
  ExprId temp{};  // LocalRef to a String temp
};
